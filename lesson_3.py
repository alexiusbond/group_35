from enum import Enum

class Color(Enum):
    RED = '\33[31m'
    GREEN = '\33[32m'
    BLUE = '\33[34m'
    YELLOW = '\33[33m'
class Drawable:
    @staticmethod
    def draw(emoji):
        print(emoji)


class MusicPlayable:
    # def __init__(self):
    #     pass

    @staticmethod
    def play_music(song):
        print(f'Now is playing {song}')

    @staticmethod
    def stop_music():
        print(f'Music stopped')


class SmartPhone(MusicPlayable, Drawable):
    pass


class Car(MusicPlayable, Drawable):
    def __init__(self, model, year, color):
        self.__model = model
        self.__year = year
        if type(color) == Color:
            self.__color = color
        else:
            raise ValueError('Color attribute must be of data type enum Color')

    @property
    def model(self):
        return self.__model

    @property
    def year(self):
        return self.__year

    @property
    def color(self):
        return self.__color

    @color.setter
    def color(self, value):
        self.__color = value

    def drive(self):
        print(f'Car {self.__model} is driving')

    def __str__(self):
        return (f'MODEL: {self.__model} YEAR: {self.__year} '
                f'COLOR: {self.__color.value}{self.__color.name} \33[0m')

    def __gt__(self, other):
        return self.__year > other.__year

    def __lt__(self, other):
        return self.__year < other.__year

    def __ge__(self, other):
        return self.__year >= other.__year

    def __le__(self, other):
        return self.__year <= other.__year

    def __eq__(self, other):
        return self.__year == other.__year

    def __ne__(self, other):
        return self.__year != other.__year


class FuelCar(Car):
    __total_fuel_amount = 1000

    @staticmethod
    def get_fuel_type():
        return 'AI 95'

    @classmethod
    def get_total_fuel_amount(cls):
        return cls.__total_fuel_amount

    @classmethod
    def fill_fuel_amount(cls, amount):
        cls.__total_fuel_amount += amount
        print(f'Fuel Cars Fabric has '
              f'{cls.get_total_fuel_amount()} liters ({cls.get_fuel_type()}).')

    def __init__(self, model, year, color, fuel_bank):
        # super().__init__(model, year, color)
        Car.__init__(self, model, year, color)
        self.__fuel_bank = fuel_bank
        FuelCar.__total_fuel_amount -= self.__fuel_bank

    @property
    def fuel_bank(self):
        return self.__fuel_bank

    @fuel_bank.setter
    def fuel_bank(self, value):
        self.__fuel_bank = value

    def drive(self):
        print(f'Car {self.model} is driving by using fuel')

    def __str__(self):
        return super().__str__() + f' FUEL BANK: {self.__fuel_bank}'

    def __add__(self, other):
        return self.__fuel_bank + other.__fuel_bank


class ElectricCar(Car):
    def __init__(self, model, year, color, battery):
        Car.__init__(self, model, year, color)
        self.__battery = battery

    @property
    def battery(self):
        return self.__battery

    @battery.setter
    def battery(self, value):
        self.__battery = value

    def drive(self):
        print(f'Car {self.model} is driving by using electricity')

    def __str__(self):
        return super().__str__() + f' BATTERY: {self.__battery}'


class HybridCar(FuelCar, ElectricCar):
    def __init__(self, model, year, color, fuel_bank, battery):
        FuelCar.__init__(self, model, year, color, fuel_bank)
        ElectricCar.__init__(self, model, year, color, battery)


print(f'Fuel Cars Fabric has {FuelCar.get_total_fuel_amount()} '
      f'liters ({FuelCar.get_fuel_type()}).')

some_car = Car('Toyota Camry', 2020, Color.RED)
print(some_car)

fuel_car = FuelCar('Ford Mustang', 2019, Color.YELLOW, 70)
print(fuel_car)

electric_car = ElectricCar('Tesla Model X', 2023,
                           Color.BLUE, 25000)
print(electric_car)

hybrid_car = HybridCar('Toyota Prius', 2007, Color.GREEN, 60, 15000)
print(hybrid_car)
hybrid_car.drive()
print(HybridCar.mro())

number_1 = 6
number_2 = 10
print(f'Number two is bigger than number one: {number_2 > number_1}')
print(f'Number one is bigger than number two: {number_1 > number_2}')
print(f'Mustang is better than Prius: {fuel_car > hybrid_car}')
print(f'Mustang is better than Prius: {fuel_car <= hybrid_car}')
print(f'Total fuel amount of banks: {fuel_car + hybrid_car} liters.')

# FuelCar.total_fuel_amount -= 100
print(f'Fuel Cars Fabric has {FuelCar.get_total_fuel_amount()} '
      f'liters ({FuelCar.get_fuel_type()}).')

FuelCar.fill_fuel_amount(500)

electric_car.play_music('Song 2')

samsung = SmartPhone()
samsung.play_music('Best Song')
samsung.stop_music()
samsung.draw('📱')

hybrid_car.draw('🚗')

if electric_car.model == 'Tesla Model X':
    print('This car is cool!')

if electric_car.color == Color.BLUE:
    print('This car is beautiful!')